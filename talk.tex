...
\frametitle{\textsc{libcarma} \href{https://github.com/AstroVPK/libcarma}{\footnotesize (https://github.com/AstroVPK/libcarma)}}
  \begin{figure}
    \includegraphics[scale=0.075]{images/CARMA(2,1)_Recovery.jpg}
  \end{figure}
  %\centering
  %\href{https://github.com/AstroVPK/libcarma}{\beamergotobutton{Link}}
\end{frame}
...
\frametitle{Conclusions}
  \begin{itemize}
    \item We need good data!
    \item Good data shows: DRW does \textbf{not work} for all AGN
    %\item PSD fits via monte carlo estimation of structure function
    %\item DRW does \textbf{not work} for all AGN
    %\item Instrumentation \textbf{not responsible} for non-DRW behavior
    \item Variability $\Rightarrow$ C-ARMA process
    %\item Inferencing $\Rightarrow$ Kalman filter
    ...
    \item TMT: Rapid sampling + low noise
    \item LSST: Long baseline
  \end{itemize}
\end{frame}
